...
void setup() 
{
  ...
  lcd.setCursor(1, 1); 
  lcd.print("W=    Rpm");
  pinMode(7, OUTPUT);                           // OC2A PWM generator
...
ISR(TIMER0_COMPA_vect)                          // timer0A compare interrupt service routine
{
  ...
  if(slot == 30)                                // 30 slot = 480000 us, 0.48 detik, interval update speed di LCD
  {
    ...
    velo = TCNT1;
    velo = velo*1;
    itoa(velo,tulisan,5);                       // convert int to decimal string
    lcd.setCursor(3, 1);                        // kolom ketiga baris kedua lcd
    lcd.print("   ");
    lcd.setCursor(3, 1); 
    lcd.print(tulisan);
    slot = 0;
